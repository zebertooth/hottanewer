<?php
class Depcustomer_model extends CI_Model{
    
    function __construct() {
        // Set table name
        $this->table = 'system_deposit';
        // Set orderable column fields
        $this->column_order = array('id','date_tran','name','bank_deposit','money','user','status');
        // Set searchable column fields
        $this->column_search = array('money','user','date_deposit');
        // Set default order
    ...
    public function getRows($postData,$manage){
		...
		$this->db->where('MONTH(date_deposit)', date('m'));
	    $this->db->where('YEAR(date_deposit)', date('Y'));
		$this->db->where('status','1');
		}elseif($manage=='week'){
...
$week_end = date('Y-m-d', strtotime('this week next sunday', $custom_date));
	$this->db->where('date_deposit >=', $week_start);
	$this->db->where('date_deposit <=', $week_end);
	$this->db->where('status','1');
		}
