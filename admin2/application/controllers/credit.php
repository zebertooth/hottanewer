...
public function wintoday(){
		...
		$withdraw = $this->topup_model->getWithdraw($user);
		$deposit = $this->topup_model->getDeposit($user);
		$profit = $withdraw-$deposit;
#หาเปอรเซ็นต์
$persent = $withdraw - $deposit;
$persent_w = $persent/$deposit*100;
if($persent_w < 0){
$show ='<span class="badge  badge-danger">'.number_format($persent_w,2).'%</span>';
}else{
$show ='<span class="badge badge-info">'.number_format($persent_w,2).'%</span>';
}
echo'<h5>ยอดได้-เสีย เดือนปัจจุบัน</h5>';
echo'
<table class="table">
  <thead class="thead-dark">
    <tr>
      ...
      <th scope="col">เปอรเซ็น</th>
    </tr>
  </thead>
  <tbody>
    <tr>
      <th scope="row">'.number_format($deposit,2).'</th>
...
echo'<h5>ยอดได้-เสีย ทั้งหมด</h5>';
echo'
<table class="table">
  <thead class="thead-dark">
    <tr>
...
$this->db->where('with_id',$r->with_id);
$this->db->update($this->config->item('system_withdraw'), $data);
#ระบบตรวจสอบโปรโมชั่นลูกค้า
if($this->topup_model->get_promotion($r->userID)==4){
$comment = 'โปรโมชั่นยอด 5,000 ถอนได้สูงสุด 500 บาท';
$this->db->set('money', '500', FALSE);
$this->db->set('comment', $comment);
$this->db->where('with_id', $r->with_id);
$this->db->update('system_withdraw');
...
#สิ้นสุดระบบอัพเดท
		...
		$this->load->model('topup_model');
		$user = $this->uri->segment(3);
		$this->balance = $this->topup_model->get_cash($user);#เหลือในเว็บ
		$this->getWithdraw = $this->topup_model->getWithdraw($user);
		$this->Totalamount = $this->uri->segment(4) + $this->topup_model->getWithdraw($user);
		$this->getDeposit = $this->topup_model->getDeposit($user);//ยอดฝาก
		$this->wheel = $this->topup_model->wheel($user);#wheel
		$this->Sumdeposit =  $this->getDeposit+$this->wheel;#ยอดรวมเครดิตก่อนส่งไป SA
		$this->sum = $this->Totalamount + $this->balance;
		$this->Total = $this->sum - $this->Sumdeposit; 
		$data['content_view']='credit/profit';
		 $this->load->view('pop',$data);
	}
...
$choice = 'week';$date = date("Y-m-d",strtotime("yesterday"));
	...
	public function getWithdraw(){
        ...
        foreach($memData as $member){
            $i++;
        $total_order = $total_order + floatval($member->money);
		//'.$member->with_id.','.$this->md5crypt->encryptIt($member->with_id).','.$member->name_with.','.$member->money.','.$member->user.','.$member->bank_deposit.','.$member->date_tran.','.$member->userID.'
		$detail='<a onclick="ModelWithReport(1491,3MSScLwtkdUbgTJ/h9g8791XYarsOYpHJCOvoVBa9Qo=,นิรันย์ พานสูงเนิน,440.00,0877919851,ธนาคารธนชาต,2020-01-23 20:16:04,1573)" class="btn btn-sm btn-secondary"><i class="fas fa-check"></i> รายละเอียด</a>';
           // $created = date( 'D j M Y', strtotime($member->dateRecieve));
//$a='<a onclick="ModelWithReport(''.$b->with_id.'',''.$this->md5crypt->encryptIt($b->with_id).'',' '.$b->name_with.'',''.$b->money.'',''.$b->user.'',''.$b->bank_deposit.'',''.$b->date_tran.'',''.$b->userID.'')" class="btn btn-danger btn-sm margin-5 text-white userinfo"><i class="fas fa-chevron-left"></i> รายละเอียด</a>';
              ...
              $data[] = array($member->user,$member->date_tran,$member->balance,$member->money,$member->name_with, $status, $member->approve, $detail);
        }
