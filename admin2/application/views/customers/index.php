...
$(document).ready(function(){
    $('#memListTable').DataTable({
        ...
        "ajax": {
            "url": "<?php echo base_url('index.php/customers/getLists'); ?>",
            "type": "POST"
        },
...
//ทำการลบเครดิตสมาชิกโดยการกระทำ
//เเก้ไขสมาชิก BtnUpdateuser
$(document).on("click", "#BtnDelcredit", function() {
        // Get form
        var form2 = $('#Addcredit')[0];
        var deeid = $("#company_id").val();
		// Create an FormData object 
        var data2 = new FormData(form2);
 $.ajax({
                        url: site_url+'/customers/delcredit_success',
                        type: 'post',
                         data: data2,
						processData: false,
						contentType: false,
                        ...
                        success: function(data){ 
   $('#result').html(data);  
  $("#BtnDelcredit").attr("disabled", true);
	$("#amounts").load(site_url+"/customers/creditRefresh/"+deeid);
    //$("#last_transaction").load(site_url+"/credit/dep_user/"+deeid);
...
// alert("The paragraph was clicked.");
                    $('#employee_detail').html(data);  
                    $('#ModalPass').modal("show");
						$("#winloss").load(site_url+"/credit/wintoday/"+company_id);
...
// เรียก Modal ข้อมูลสมาชิกเพื่อเเก้ไข
$( "table" ).delegate( ".view_api", "click", function() {
          var company_id = $(this).attr("id");  
 $.ajax({
                        url: site_url+'/customers/apiedit',
                        type: 'post',
                        data:{company_id:company_id},  
                        success: function(data){ 
// alert("The paragraph was clicked.");
...
//ฟังก์ชั่นโหลดข้อมูล
   function wintoday(){
   	var deeid = $("#company_id").val();
   	$("#winloss").load(site_url+"/credit/wintoday/"+deeid);
   }
   function winmonth(){
  	var deeid = $("#company_id").val();
   	$("#winloss").load(site_url+"/credit/winall/"+deeid);
   
   }
//สิ้นสุดฟังชั่นโหลดข้อมูล
function myFunction() {
  /* Get the text field */
