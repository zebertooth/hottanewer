...
error_reporting(0);
$query_sum="select SUM(balance) from topup_truncate where cancel_ok='0' and send_ok='0'";
$result_sum=mysql_query($query_sum);
while($row = mysql_fetch_array($result_sum)){ echo $row['balance']. " = ". $row['SUM(balance)']."บาท";  }
	?>
    </div>
	<!-- Datepicker -->


	<h1>รายการคืนไม่สำเร็จ รอคืนเงิน</h1>
...
<table class="table table-striped table-bordered table-hover table-condensed">
	...
	foreach($q->result() as $k => $r){
		...
		if($r->network==0){
			...
			if($q2->num_rows()>0){
				$r2=$q2->row();
				echo '<td colspan="7"><strong>'.$r2->name.'</strong> TaxID:'.$r2->txid.'</td><td><a href="javascript:bills_cancel(\''.$r->shop.'\',\''.$r->tc_id.'\');" class="big"><span class="glyphicon glyphicon-remove-circle"></span></a></td></tr>';
			}else{
...
$(function(){
	$('#datepicker').change(function (){
		...
		var x = result[2]+"-"+result[0]+"-"+result[1];//new order date from datepicker
		 location.replace(site_url+'/topup/cancel_list/'+x);
	});	
});
...
function bills_cancel(com_id,id){
	if(confirm('Do you want to cancel '+id+' ?')){
		$.ajax({type:'POST',url:site_url+'/topup/bills_cancel',data:'id1='+com_id+'&id2='+id,success:function(data){
			if(data=='1'){location.reload();}else{alert(data);}
		}});
	}
}
